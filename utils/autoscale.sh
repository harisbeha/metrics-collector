...
deployment=""
AUTOSCALING='
              3|10|5|prod-main|prod-main-celerymediadownload|celerymediadownload|lmodzuns|media_download;
              3|5|3|prod-main|prod-main-celerybackgroundmediadownload|celerybackgroundmediadownload|lmodzuns|background_media_download'
RABBIT_HOST=''
RABBIT_USER=''
RABBIT_PASS=''
...
while true; do
  for autoscaler in "${autoscalingArr[@]}"; do
    IFS='|' read minPods maxPods mesgPerPod namespace deployment appName vHost queueName <<< "$autoscaler"

    queueMessagesJson=$(curl -s -S --retry 3 --retry-delay 3 -u $RABBIT_HOST:$RABBIT_PASS https://bobbish-hare.rmq.cloudamqp.com/api/queues/lmodzuns/$queueName)
    ...
    if [[ $? -eq 0 ]]; then
        ...
        if [[ $currentPods != "" ]]; then
          if [[ $requiredPods -ne $currentPods ]]; then
            ...
            if [[ $scale -eq 0 ]]; then
              ...
              deis scale -a $namespace $appName=$desiredPods > /dev/null
      ...
      echo "$(date) -- Failed to get queue messages from $RABBIT_HOST for $vHost and $appName and $deployment."
      notifySlack "Failed to get queue messages from $RABBIT_HOST for $vHost and $deployment."
    fi
  done
