...
def create_queue_metric(queue_name):
    metric_name = 'custom.googleapis.com/rabbit_{0}_total_count'.format(queue_name)
...
async def run_check():
    query_url = 'https://{0}:{1}@{2}.rmq.cloudamqp.com/api/queues?page=1&page_size=100&name=(hitmansync|autonomousactions|slowtasks|hitman|payment|integrations|actions|integrationsmt|workmarket|partners|mergeeldata|autocap|celery|kaltura|priorityautocap|workflowexecution|ftp|cv24|metrics|throttler)&use_regex=true&pagination=true'.format(RABBIT_USER, RABBIT_PASS, RABBIT_HOST)
    ...
    for item in items:
        ...
        if '@' in queue_name or 'pidbox' in queue_name:
            pass
        else:
            total_metric = create_queue_metric(queue_name)
            client.write_point(total_metric, resource, total_count)
...
async def check_rabbit():
    """
    Collect and send Rabbit MQ queues counts to StackDriver 
