...
def create_queue_metric():
    metric_name = 'custom.googleapis.com/rabbit_media_download_combined_total_count'
...
async def run_check():
    query_url = 'https://{0}:{1}@{2}.rmq.cloudamqp.com/api/queues?page=1&page_size=100&name=(media_download|priority_media_download|background_media_download)&use_regex=true&pagination=true'.format(RABBIT_USER, RABBIT_PASS, RABBIT_HOST)
...
async def check_rabbit():
    """
    Collect and send Rabbit queue message counts to StackDriver
